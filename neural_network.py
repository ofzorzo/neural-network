...
class NeuralNetwork:
    ...
    def backpropagation_numerically_verified(self, train):
        b_grad = self.backpropagation(train)
        n_grad = self.compute_numerical_verification(train)
        self.print_numerical_verification(n_grad)
        print("Verificando corretude dos gradientes com base nos gradientes numericos:")
        #print(b_grad)
        backprop = []
        for layer in range(0, len(b_grad)):
            sum = 0
            for next in range(0, len(b_grad[layer])): #escolhe linha da matriz, que representa o neuronio onde os pesos da linha vão chegar
                for weight in range(0, len(b_grad[layer][next][0])):
                    sum = sum + b_grad[layer][next][0][weight]
            backprop.append(sum)
        #print(backprop)
        for k in range(0, len(n_grad)):
            print("\tErro entre gradiente via backprop e gradiente numerico para Theta" + str(k+1) + ": ", end="")
            numerical = 0
            for i in range(0, len(n_grad[k])):
                for j in range(0, n_grad[k][i].size):                    
                    numerical = numerical + n_grad[k][i,j]
            print(numerical)
            print(pow(backprop[k], 2) - pow(2,numerical))
...
nn = NeuralNetwork(s, w, epsilon)
            
nn.backpropagation(train)
numerical_gradients = nn.compute_numerical_verification(train)
nn.print_numerical_verification(numerical_gradients)
#nn.backpropagation_numerically_verified(train) #nao faco a menor ideia de como eles tao fazendo a comparacao
#print("\n\n")
#nn.print_network()